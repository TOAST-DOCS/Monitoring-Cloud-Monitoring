...
- **사용**: 템플릿 선택 영역이 노출됩니다. 서비스를 선택하면 해당 서비스에 대한 템플릿 목록이 표시됩니다.
...
집계를 지원하는 지표에서만 집계 설정 영역이 노출됩니다.
...
**동적 필터 적용** 토글을 통해 대시보드 상단에서 선택한 동적 필터 조건을 해당 위젯에 반영할지 여부를 설정할 수 있습니다. 기본값은 **On**입니다. 동적 필터 적용 시 항상 대시보드 상단 필터에서 선택한 조건으로 적용됩니다.
...
동적 필터는 대시보드 상단에 위치한 공통 필터 영역을 통해 개별 위젯을 하나씩 수정할 필요 없이 전체 위젯의 데이터를 한 번에 필터링하는 기능입니다.

동적 필터는 서비스별로 그룹화되어 배치됩니다. 필터 항목 간 영향도에 따라 내부 정의된 우선순위에 따라 좌측에서 우측으로 자동 배치됩니다.

동적 필터 항목을 변경하면 대시보드 내 위젯에 즉시 반영됩니다. 좌측 필터의 선택값을 변경하면 우측 필터의 항목이 변경된 조건에 맞게 갱신되며, 우측 필터의 선택 상태는 초기화됩니다.
...
조회 모드에서 **동적 필터 관리** 버튼을 클릭하면 동적 필터 관리 모달이 열립니다. 편집 모드에서는 동적 필터 관리 버튼이 노출되지 않습니다.
...
!!! tip "알아두기"
    ...
    신규 Agent 지원 지표는 [신규 Agent 설치 가이드](new-instance-metric-ncgn.md)를 참고하십시오.
...
집계를 지원하는 지표에서는 쿼리 설정 블록 내에 집계 설정 영역이 노출됩니다. **사용**을 선택하면 집계 항목(평균, 최소, 최대)을 선택할 수 있으며, **사용 안 함**(기본값)을 선택하면 원본 데이터를 기반으로 알림을 판단합니다.
...
알림명, 발생 일시, 종료 일시, 서비스, 리소스, 지표 항목, 집계, 결과값, 지속 시간을 확인할 수 있습니다. 집계를 지원하는 지표의 경우 설정된 집계값이 표시되며, 집계를 지원하지 않는 지표의 경우 `-`로 표시됩니다. 발생 일시, 종료 일시로 테이블 정렬을 변경할 수 있습니다.
...
알림 발송 이력 보기 모달에서는 해당 알림의 **발생일시**, **지속시간**, **지표 항목**, **집계**, **결과값**, **알림 조건** 등의 알림 설정 정보와 **발생 위치**를 상세하게 확인할 수 있습니다.
