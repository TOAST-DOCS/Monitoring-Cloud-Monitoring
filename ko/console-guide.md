...
동적 필터는 대시보드 상단에서 필터를 선택하면 대시보드 내의 여러 위젯에 동시에 적용되는 기능입니다.
위젯마다 개별적으로 필터를 설정하지 않아도 대시보드 수준에서 리전, 인스턴스 등의 조건을 손쉽게 변경하며 지표를 확인할 수 있습니다.

##### 동적 필터 관리

대시보드 헤더 영역의 **동적 필터 관리**를 클릭하면 동적 필터 관리 화면이 열립니다.

**+ 추가**를 클릭하여 동적 필터를 하나씩 추가할 수 있습니다. 각 항목은 다음 정보를 설정합니다.
1. **필터 이름**: 대시보드 상단에 표시되는 필터의 이름을 입력합니다.
2. **서비스**: 동적 필터를 적용할 서비스를 선택합니다.
3. **레이블**: 선택한 서비스에서 사용 가능한 레이블을 선택합니다.
   - 예: Instance 서비스 선택 시 **리전**, **인스턴스** 등의 레이블을 선택할 수 있습니다.

하나의 대시보드에 여러 서비스의 동적 필터를 추가할 수 있습니다.
동적 필터를 추가하면 대시보드 상단에 서비스별로 필터 드롭다운이 배치되며, 상위 필터의 값을 선택하면 하위 필터의 조건 값 목록이 자동으로 갱신됩니다.

위젯 추가 또는 수정 시 **동적 필터 적용** 토글을 활성화하면 대시보드 상단에서 선택한 동적 필터 조건이 해당 위젯에 적용됩니다.
