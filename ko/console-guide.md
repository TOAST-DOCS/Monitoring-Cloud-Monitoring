...
데이터 간격은 차트에 표시되는 데이터 포인트 사이의 시간 간격입니다. 조회 기간에 따라 자동으로 조정되며, 차트의 가독성과 성능을 최적화합니다.
...
> **참고**: 조회 기간이 길어질수록 데이터 간격이 넓어지며, 이 경우 세부적인 데이터 변동을 확인하기 어려울 수 있습니다.

##### 집계 기능

![집계 기능 선택 화면](../images/aggregation_select_ui.png)

지표 항목을 선택한 후 집계 기능을 설정할 수 있습니다. 집계 기능은 조회 기간 동안의 지표 데이터를 특정 방식으로 집계하여 표시합니다.

예를 들어, 지표 수집 주기가 1분이고 데이터 간격이 5분인 경우 차트에는 5분 간격의 데이터만 표시됩니다.
이때 5분 구간 내의 세부 데이터(1분 간격)를 직접 확인하기 어렵습니다.
집계 기능을 사용하면 해당 구간 내의 최대값, 최소값, 평균값을 함께 확인할 수 있어 데이터의 변동 범위를 파악하는 데 유용합니다.

| 집계 방식 | 설명 |
| --- | --- |
| 최대 | 조회 기간 동안의 최대값을 표시합니다. |
| 최소 | 조회 기간 동안의 최소값을 표시합니다. |
| 평균 | 조회 기간 동안의 평균값을 표시합니다. |

아래는 CPU 사용률 지표에 집계 기능을 적용한 예시입니다. 지표 수집 주기는 1분이고, 데이터 간격은 5분입니다.
...
| --- | --- |
| 1-5분 | 65, 32, 26, 72, 42 |
| 6-10분 | 40, 39, 33, 72, 31 |
| 11-15분 | 68, 72, 82, 59, 30 |
| 16-20분 | 62, 52, 27, 26, 30 |
...
![집계 기능 적용 예시](../images/aggregation_example_chart.png)

위 그래프는 원본 지표 데이터(회색)와 함께 최대(빨간색 실선), 최소(초록색 점선), 평균(주황색 대시선)이 표시된 예시입니다. 각 5분 구간의 데이터가 해당 집계 방식에 따라 계산되어 표시됩니다.
...
알림 조건에서 집계 기능을 설정하면 지표 데이터를 집계하여 알림 발생 여부를 판단합니다.
일시적인 스파이크나 순간적인 변동으로 인한 불필요한 알림을 방지하는 데 유용합니다.

| 집계 방식 | 설명 |
| --- | --- |
| 최대 | 최근 수집된 데이터 중 최대값을 기준으로 조건을 판단합니다. |
| 최소 | 최근 수집된 데이터 중 최소값을 기준으로 조건을 판단합니다. |
| 평균 | 최근 수집된 데이터의 평균값을 기준으로 조건을 판단합니다. |

> **참고**: 집계 기능을 사용하지 않으면 순간 값을 기준으로 조건을 판단합니다.

**활용 시나리오**

| 집계 방식 | 활용 시나리오 |
| --- | --- |
| 최대 | 순간적인 피크도 놓치지 않고 감지해야 할 때 |
| 최소 | 리소스 가용량이 특정 수준 이하로 떨어지는 것을 감지할 때 |
| 평균 | 일시적 변동을 무시하고 안정적인 알림을 받고 싶을 때 (권장) |
