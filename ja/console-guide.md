<!-- machine_translated: true -->

<!-- pre-align:aligned sig=503aff601f8b -->

<a id="monitoring-cloud-monitoring-console-user-guide"></a>
## Monitoring > Cloud Monitoring > コンソール使用ガイド { #monitoring-cloud-monitoring-console-user-guide }

コンソール使用ガイドでは、Cloud Monitoringサービスの基本的な使用方法を案内します。

<a id="dashboard"></a>
## ダッシュボード { #dashboard }

**Monitoring > Cloud Monitoring > ダッシュボード**では、NHN Cloudサービスで作成したリソースから収集した様々な指標をチャートウィジェットで作成し、ダッシュボードを構成して指標を確認できます。

![照会期間自動更新](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01-1.png)

ユーザーが構成したダッシュボードの指標データの照会期間は、基本的に現在時刻から5分前までのデータを照会します。基本設定された自動更新間隔は1分です。

![照会期間ユーザー指定](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01-2.png)

ユーザー指定期間を設定すると、自動更新は使用できません。

<a id="view-modeedit-mode"></a>
### 照会モード/編集モード { #view-modeedit-mode }

![ダッシュボード照会モード](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_01-1.png)

![ダッシュボード編集モード](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_01-2.png)

ダッシュボードは照会モードと編集モードの2つのモードがあります。

![照会モードヘッダ](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_01-3.png)

ダッシュボードは基本的に照会モードで表示され、ダッシュボードやウィジェットを修正するには、編集モードのトグルをクリックして編集モードに切り替える必要があります。

![編集モードヘッダ](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_01-4.png)

編集モードでは、ダッシュボードの名前や説明を修正したり、ダッシュボードを複製または削除できます。各ウィジェットを修正したり、ウィジェットの位置移動や複製を行うこともできます。

![全体保存通知モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_01-5.png)

ウィジェットを追加したり、ウィジェットの位置やサイズなどを修正した内容は、**全体保存**をクリックすることで適用されます。保存しなかった場合、保存通知モーダルが表示され、保存しなかった変更内容は消えます。

<a id="create-a-dashboard"></a>
### ダッシュボードの作成 { #create-a-dashboard }

![ダッシュボード追加モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_02-1.png)

照会モードで**ダッシュボード**タブの右側の**+**ボタンをクリックすると、ダッシュボード追加モーダルが開きます。

![空のダッシュボード](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_02-2.png)

作成されたダッシュボードがない場合、**+ダッシュボード作成**ボタンが表示されます。

![ダッシュボード作成後](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_02-3.png)

**ダッシュボード名**と**ダッシュボード説明**を入力してください。**ダッシュボード名**は必ず入力しなければならない必須値であり、重複した名前で設定できますが、なるべくユニークな名前を使用することを推奨します。
**ダッシュボードの説明**は選択値で、そのダッシュボードを説明できる内容を入力します。

<a id="create-a-dashboard-dashboard-template"></a>
#### ダッシュボードテンプレート

ダッシュボードを独自に構成するか、あらかじめ構成されたテンプレートから開始できます。**ダッシュボードテンプレート**項目で、使用有無を選択します。

- **使用**：テンプレートの選択領域が表示されます。サービスを選択すると、該当サービスのテンプレート一覧が表示されます。
- **使用しない**(既定値)：テンプレートを使用せず、空のダッシュボードを作成します。

テンプレートの使用を選択した場合は、サービスとテンプレートをそれぞれ選択します。テンプレートを選択すると、ダッシュボード名フィールドにテンプレート名が自動入力されます。名前は自動入力された後でも変更できます。

テンプレート一覧から**プレビュー**ボタンをクリックすると、テンプレートのプレビューモーダルが開きます。

プレビューモーダルでは、テンプレートに含まれるウィジェットの構成とレイアウトを事前に確認できます。プレビュー画面にはサンプルデータが表示され、ダッシュボードの作成後に実際のデータを確認できます。

<a id="set-up-a-dashboard"></a>
### ダッシュボード構成 { #set-up-a-dashboard }

![ダッシュボード例](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03-1.png)

ダッシュボードにはウィジェットとウィジェットグループを追加できます。ウィジェットはダッシュボードごとに12個まで追加でき、追加するとダッシュボードの最下段に追加されます。

![ウィジェット例](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03-2.png)

ウィジェットはダッシュボードを構成する最小単位です。**+ ウィジェットの追加**をクリックするか、そのボタンのドロップダウンリストから**ウィジェットの追加**、**ウィジェットテンプレートの追加**を選択して作成できます。ウィジェットのサイズと位置は、ダッシュボード編集モードで自由に変更できます。

![ウィジェットグループ例](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03-3.png)

ウィジェットグループを使用すると、複数のウィジェットをグループ化して管理できます。ウィジェットグループを折りたたんだり広げたりして、複数のウィジェットを隠したり、表示したりすることができ、ウィジェットグループの高さと幅はグループ内のウィジェットサイズに応じて自動的に調整されます。

<a id="set-up-a-dashboard-addedit-a-widget-and-widget-group"></a>
#### ウィジェット及びウィジェットグループの追加/修正

![ウィジェット追加ボタン領域](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a-1.png)

ウィジェットおよびウィジェットグループは、照会モードと編集モードの両方で追加することができます。ただし、ウィジェットおよびウィジェットグループを修正するには、編集モードに切り替える必要があります。

##### ウィジェットの追加

![ウィジェット追加画面の基本情報](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a_01-1.png)

ウィジェットの追加で、好きな指標とフィルタを適用して自由にチャートウィジェットを作成できます。
ウィジェット名、グラフタイプ、サービス、指標設定 > 指標の項目は必須項目なので、必ず入力してください。
一つのウィジェットは単一のサービスのみ表現できます。ただし、指標項目は複数選択できます。

![ウィジェット追加画面指標項目選択後](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a_01-2.png)

指標項目を選択するたびに、画面下部に指標項目のフィルタと凡例を設定できる要素が追加されます。
指標のフィルタラベルは種類ごとに1つだけ指定可能で、すべてのフィルタは「&」演算の形で最終適用されます。したがって、フィルタを複数適用する場合、条件値の検索結果は、以前に設定したフィルタ条件をすべて満たす検索結果のみ表示されます。先に追加したフィルタの演算子と条件値によって、選択できる条件値がなくなる場合があります。

凡例は、各指標項目で照会した指標データを区別するための名前です。指標項目を追加すると、既定で推奨される名前が自動的に入力されます。`{{ }}`のように二重中括弧の構文で囲まれたプレースホルダーの前後に、任意のテキストを追加できます。テキストを何も入力しない場合は、指標項目が持つラベルプロパティを全て連結して表示します。
単位を選択して、チャートに表示するY軸のタイトル及び単位を指定できます。単位に応じた個別のフォーマット設定はサポートしておらず、指標項目の実際の値を小数点以下15桁まで表示します(16桁目で四捨五入されます。ただし、`Percent(0.0~1.0)`を選択した場合のみ、例外として0~100の値に変換して表示します)。Y軸の位置設定では、指標項目のY軸の位置を**自動** / **左** / **右**から選択できます。指標項目の追加時は、既定で「自動」が選択されています。全ての指標項目が「自動」に設定されている場合、順次Y軸を左または右に分けて配置します。Y軸の位置が左または右に指定されている指標がある場合は、同じY軸の方向に配置されます。このとき、単位とY軸の位置が同じ指標項目があれば、同じY軸で表現します。

###### クエリ設定ブロック

指標項目を選択すると、該当指標の指標設定ブロックが追加されます。この指標設定ブロック内には、クエリ設定ブロックが標準で1つ含まれます。

クエリ設定ブロックは、個々のクエリごとにフィルタ、凡例、集計などを設定する単位です。1つの指標設定ブロック内に複数のクエリ設定ブロックを追加でき、各クエリにはアルファベット順(A、B、C...)で自動的に番号が割り当てられます。

- **追加**：指標設定ブロックの**+**アイコンをクリックすると、下部にクエリ設定ブロックが追加されます。
- **コピー**：クエリ設定ブロックのコピーアイコンをクリックすると、フィルタ、凡例、集計の設定値が全て複製されたクエリが追加されます。
- **削除**：クエリ設定ブロックが2つ以上ある場合、削除アイコンが有効になります。
- **折りたたみ/展開**：クエリ設定ブロックを折りたたむと、タイトル領域にフィルタや集計などの主要な要約情報が表示されます。

クエリ設定ブロック間で、フィルタや集計などの設定値を重複させることができます。それぞれ独立したクエリとみなされるため、設定値が同じ場合でもウィジェットに全て表示されます。

###### 集計設定

照会期間が長くなると、チャートに表示されるデータ間隔が自動的に調整されます。このとき、データ間隔内に存在する個別のデータポイントは、チャートに反映されない場合があります。
集計機能を使用すると、データ間隔内の全てのデータを平均、最小、最大などの集計関数で計算し、チャートに表示できます。
集計をサポートする指標でのみ、集計設定領域が表示されます。

- **使用しない**(既定値)：各時点の実際のデータ値を加工せずにそのまま表示します。照会期間によっては、データ間隔内の一部のデータがチャートに反映されない場合があります。
- **使用**：集計項目(平均、最小、最大)を選択できます。データ間隔内に発生した全てのデータを集計して反映するため、全体的な流れや傾向を把握しやすくなります。

照会期間に応じたデータ間隔は次のとおりです。

| 照会期間 | データ間隔 |
|---------|----------|
| ~ 5分 | 15秒 |
| ~ 30分 | 30秒 |
| ~ 1時間 | 1分 |
| ~ 6時間 | 5分 |
| ~ 12時間 | 10分 |
| ~ 24時間 | 20分 |
| ~ 2日 | 30分 |
| ~ 7日 | 2時間 |
| ~ 14日 | 3時間 |
| ~ 27日 | 6時間 |
| ~ 366日 | 24時間 |

例えば、照会期間を24時間に設定するとデータ間隔が20分になり、その間のデータはチャートに直接表示されません。集計を使用すると、20分区間内の全てのデータを反映できます。

以下は、5分のデータ間隔で集計を適用した例です。灰色の点は収集された元のデータであり、赤い点は実際のチャートに表示される値です。

![集計適用の比較](https://static.toastoven.net/prod_cloud_monitoring/20260407_aggregation_compare_en.png)

Aggregation Offは、集計を使用しない場合です。12:05と12:10時点のデータのみがチャートに表示され、その間に収集されたデータ(0.8、5.0、1.0)はチャートで確認できません。
Aggregation Onは、平均集計を使用した場合です。各区間内の全てのデータの平均を計算してチャートに表示するため、区間内に収集されたデータも結果に反映されます。


###### 動的フィルタ適用

**動的フィルタ適用**トグルで、ダッシュボード上部で選択した動的フィルタ条件を該当ウィジェットに反映するかを設定できます。既定値は**On**です。動的フィルタ適用時は、常にダッシュボード上部のフィルタで選択した条件が適用されます。

![ウィジェット追加画面プレビューレイヤー](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a_01-3.png)

グラフタイプを変更したり、指標項目フィルタを修正した後、変更点が反映されたチャートウィジェットの様子をプレビューできます。画面右上の**プレビュー**トグルをクリックすると、フィルタと凡例値を適用したチャートウィジェットをリアルタイムで確認できるプレビューレイヤーを開閉できます。プレビューレイヤーは左右の幅を調整できます。
##### ウィジェットテンプレートの追加

![ウィジェットテンプレート追加モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a_02-1.png)

ウィジェットテンプレートは、あらかじめ設定したウィジェットの指標項目を利用してウィジェットを簡単に作成できる機能です。ウィジェットを追加した後、ウィジェット修正を通じて指標項目にフィルタを追加したり、指標項目や名前を変更できます。
ただし、一部のウィジェットテンプレートで追加したウィジェットは削除のみ可能で、修正ができない場合があります。

##### ウィジェットグループ追加

![ウィジェットグループ追加モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a_03-1.png)

ダッシュボードにウィジェットグループを追加できます。ウィジェットをウィジェットグループに追加するには、編集モードでウィジェット上部のタイトル領域をドラッグしてウィジェットグループ内に移動します。

ウィジェットグループはウィジェットと同様に修正が可能で、ウィジェットグループを修正するには編集モードに切り替える必要があります。

![ウィジェットグループコンテキストメニュー](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_a_03-2.png)

- **修正**をクリックすると、**ウィジェットグループの修正**モーダルが開きます。
- **グループ削除**をクリックすると、ウィジェットの位置やサイズ情報はそのまま残り、グループのみ削除されます。
- **グループ及び下位ウィジェット全体削除**をクリックすると、グループ内のウィジェットまで削除されます。

<a id="set-up-a-dashboard-moveclonedelete-a-widget"></a>
#### ウィジェットの移動/複製/削除

![ウィジェットサイズ調整](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_b_01-1.gif)

編集モードに切り替えた後、ウィジェットの右下隅をドラッグしてサイズを調整したり、ウィジェットの上部領域をドラッグして位置を移動させることができます。

![ウィジェットコンテキストメニュー](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_b_01-3.png)

- **削除**をクリックしてウィジェットを削除できます。
- **ダッシュボード内のウィジェット複製**をクリックしてダッシュボードにウィジェットを複製できます。複製されたウィジェットは最下段に追加されます。
- **他のダッシュボードにウィジェットを複製**をクリックして他のダッシュボードにウィジェットを複製できます。この時、ウィジェットを追加するダッシュボードを選択できるモーダルが開きます。

![他のダッシュボードにウィジェット複製モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_b_01-2.png)

<a id="set-up-a-dashboard-create-a-notification"></a>
#### 通知作成

![ウィジェットコンテキストメニュー](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_b_01-3.png)

**通知作成**をクリックすると、**通知管理** > **通知作成**画面に移動します。ウィジェット名、サービス、指標項目、指標項目フィルタ情報など、当該ウィジェットの情報が入力された状態で簡単に通知を作成できます。
通知条件と通知受信対象を入力するだけで、簡単に通知を作成できます。

![ウィジェット通知作成でアクセスした場合](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_03_c-1.png)

<a id="dynamic-filter"></a>
### 動的フィルタ { #dynamic-filter }

動的フィルタは、ダッシュボード上部の共通フィルタ領域から、個別のウィジェットを1つずつ変更することなく、全てのウィジェットのデータを一括でフィルタリングする機能です。

動的フィルタはサービスごとにグループ化されて配置されます。フィルタ項目間の影響度に基づく内部定義の優先順位に従い、左から右へ自動的に配置されます。

動的フィルタ項目を変更すると、ダッシュボード内のウィジェットに即時反映されます。左側のフィルタの選択値を変更すると、右側のフィルタ項目が変更された条件に合わせて更新され、右側のフィルタの選択状態は初期化されます。

動的フィルタの選択状態に応じて、以下のように表示されます。

| 選択状態 | 表示形式 | 例 |
|---------|---------|------|
| 全て選択(既定値) | `全て` | 全て |
| 1つ選択 | 選択したフィルタ値 | 韓国(パンギョ) |
| 2つ以上選択 | `{フィルタ値} ほか N個` | 韓国(パンギョ) 他 2個 |
| 任意項目なし | `任意項目なし` | 任意項目なし |
| 項目なし | `項目なし` | 項目なし |

> 参考：動的フィルタは、ウィジェットの追加/編集画面で**動的フィルタ適用**トグルを有効にしたウィジェットにのみ適用されます。

<a id="dynamic-filter-management"></a>
#### 動的フィルタの管理

照会モードで**動的フィルタの管理**ボタンをクリックすると、動的フィルタ管理モーダルが開きます。編集モードでは、動的フィルタ管理ボタンは表示されません。

動的フィルタはダッシュボードごとに独立して管理されます。モーダル上部に、現在設定中の動的フィルタが適用されるダッシュボード名が表示されます。

- **追加**：**追加**ボタンをクリックすると、動的フィルタ一覧の一番下に新しい行が追加されます。以下の項目を入力します。
- **フィルタ名**：ダッシュボード上部に表示されるフィルタ名を入力します。
- **サービス**：動的フィルタを適用するサービスを選択します。(例：Instance、Load Balancer)
- **ラベル**：選択したサービスで使用可能なラベルを選択します。(例：Instanceサービスを選択した場合はリージョン、インスタンスなど)
- **編集**：既存の動的フィルタの**編集**ボタンをクリックすると、フィルタ名、サービス、ラベルを変更できます。
- **削除**：**削除**ボタンをクリックすると、該当の動的フィルタが削除されます。

同じサービスとラベルの組み合わせは重複して追加できません。

!!! tip 「ポイント」
Instanceサービスの動的フィルタでインスタンスを選択する際、既存のAgentと新規Agentがインストールされたインスタンスの両方が表示されます。
Agentのタイプによってサポートされる指標が異なるため、選択したインスタンスのAgentタイプに該当しない指標はデータが表示されない場合があります。
新規Agentがサポートする指標については、[新規Agentインストールガイド](new-instance-metric.md)をご参照ください。

<a id="manage-a-dashboard"></a>
### ダッシュボード管理 { #manage-a-dashboard }

![照会モードヘッダ](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_04-1.png)

![ダッシュボード管理モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_04-2.png)

**ダッシュボード管理**をクリックすると、ダッシュボード管理モーダルが表示されます。ダッシュボードの名前、説明、作成日時、プロジェクトダッシュボードの公開設定有無を確認したり、編集することができます。

![ダッシュボード管理モーダル > 編集モード](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_04-3.png)

各ダッシュボードの**編集**列の修正または削除アイコンをクリックして、そのダッシュボードの名前と説明を修正したり、ダッシュボードを削除することができます。
**プロジェクトダッシュボードの表示設定**トグルをクリックして、プロジェクトカスタムダッシュボード画面でそのダッシュボードの表示の有無を設定できます。公開設定を解除しても、Cloud Monitoringサービスでは常に公開されます。

<a id="download-widget-data"></a>
### ウィジェットデータのダウンロード { #download-widget-data }

![ウィジェットデータダウンロードメニュー](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_01_05-1.png)

ダッシュボードのすべてのウィジェットデータを.png、.csv、.xlsxファイルでダウンロードできます。ファイル名はダッシュボード名で保存され、.csvファイルの場合、ダッシュボード名で圧縮した.zipファイルでダウンロードされます。内部には、各ウィジェット名の個別.csvファイルが含まれています。
.csv、.xlsxファイルの場合、開始日時を基準に最大3か月間のデータのみダウンロードできます。照会期間が1か月未満の場合、5分間隔でデータを提供し、1か月以上は1日間隔でデータを提供します。
指定した照会期間が長かったり、ウィジェットとウィジェットに含まれる指標データが多いほど、ダウンロードにかかる時間が長くなる場合があります。

<a id="manage-notifications"></a>
## 通知管理 { #manage-notifications }

**Monitoring > Cloud Monitoring > 通知設定**では、NHN Cloudリソースに通知を追加することができ、通知の発生履歴を確認できます。

![通知設定リスト画面](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_01-1.png)

<a id="notification-settings"></a>
### 通知設定 { #notification-settings }

![通知設定リスト画面 - 作成済み](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_01_a-4.png)

通知設定画面では、作成した通知の情報を確認できるテーブルが表示され、通知名、通知の使用可否、作成日時でテーブルのソートを変更できます。
- **通知の使用有無**トグルをクリックして、通知を有効または無効にできます。
- **通知詳細表示**列の**表示**をクリックすると、**通知詳細情報**モーダルが表示されます。
- **修正**列の**修正**をクリックすると、**通知修正**画面に移動します。
- 各通知行をクリックすると、**通知発生履歴**タブに移動し、その通知が自動照会されて表示されます。

<a id="notification-settings-createedit-notifications"></a>
#### 通知の作成/修正

![通知作成/修正画面](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_01_a-1.png)

通知基本情報の名前、サービスと通知設定の指標項目と通知条件、通知の受信対象は必須値なので必ず入力する必要があります。
通知のサービスは単一選択のみ可能で、指標項目は複数選択できます。

![通知作成/修正画面 - 指標選択する](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_01_a-2.png)

指標項目を選択すると、該当指標の指標設定ブロックが追加され、指標設定ブロック内にはクエリ設定ブロックが標準で1つ含まれます。クエリ設定ブロックは、個々のクエリごとにフィルタ、集計、条件などを設定する単位であり、1つの指標設定ブロック内に複数のクエリ設定ブロックを追加できます。

集計をサポートする指標では、クエリ設定ブロック内に集計設定領域が表示されます。**使用**を選択すると、集計項目(平均、最小、最大)を選択でき、**使用しない**(既定値)を選択すると、元のデータに基づいて通知を判定します。

指標項目のフィルタは入力しなくてもよいですが、条件は必ず1つ以上入力する必要があります。通知を受けたい条件を比較方法、しきい値、持続時間の組み合わせで設定します。

![通知設定リスト画面 - 通知受信対象](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_01_a-3.png)

通知受信対象は1つ以上選択する必要があります。通知受信対象タイプは、通知受信グループのみサポートされ、**プロジェクト管理 > 通知受信グループ管理** 画面で管理できます。[[ガイドへ]](/nhncloud/ja/console-guide/#_34)

通知受信グループのカスタムWebフックで通知を受け取ることができます。

- カスタムWebフックリクエストデータとして提供するパラメータリスト

| 値 | 説明 | タイプ | 備考 |
| --- | --- | --- | --- |
| orgName | 組織名 | String |  |
| projectName | プロジェクト名 | String |  |
| tenantType | サービスタイプ | String | `organization`(カスタムダッシュボード)<br>`project`(Cloud Monitoring) |
| referenceKey | 組織またはプロジェクトID | String |  |
| serviceName | サービス名 | String |  |
| alertName | 通知名 | String |  |
| alertId | 通知ID | String |  |
| eventsCount | 発生イベント数 | Integer |  |
| events | 通知イベント | List<Object\> | eventsパラメータリスト参照 |

- eventsパラメータリスト

| 値 | 説明 | タイプ | 備考 |
| --- | --- | --- | --- |
| events[].resourceTypeName | リソースタイプ名(En) | String | |
| events[].enMetricName | 指標名(En) | String | |
| events[].threshold | しきい値 | String | Double形式の文字列 |
| events[].operator | 比較方法 | String | Enum (`EQUAL`, `NOT_EQUAL`, `GREATER_THAN`, `LESS_THAN`, `GREATER_THAN_OR_EQUAL`, `LESS_THAN_OR_EQUAL`) |
| events[].duration | 持続時間 | String | 条件に設定された値 |
| events[].result | 検出された値 | String | Double形式の文字列 |
| events[].startAt | イベント発生日時 | String | ex) `2024-10-29T08:44:22Z` |
| events[].endAt | イベント終了日時 | String | ex) `2024-10-29T08:44:22Z` |
| events[].contMinutes | イベント持続時間(分) | Integer | イベントが発生した時間と現在時間との差(分) |
| events[].labels | 発生位置 | Map<String, String\> | |

<a id="notification-settings-view-notification-details-modal"></a>
#### 通知詳細表示モーダル

![通知詳細表示モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_01_b-1.png)

通知詳細表示モーダルでは、設定した通知の詳細情報を確認できます。通知名、説明、サービス、指標項目のフィルタ及び通知条件、集計設定情報、通知受信対象を確認できます。集計をサポートする指標の場合は、設定された集計値(使用しない/平均/最大/最小)が表示され、集計をサポートしない指標の場合は`-`と表示されます。

<a id="notification-history"></a>
### 通知発生履歴 { #notification-history }

![通知発生履歴照会画面](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_02-1.png)

発生した通知の履歴を照会できる検索フィルタとテーブルを使用して発生した通知を多様に照会することができ、検索フィルタで照会した通知情報を確認できるテーブルを提供します。
通知名、発生日時、終了日時、サービス、リソース、指標項目、集計、結果値、継続時間を確認できます。集計をサポートする指標の場合は、設定された集計値が表示され、集計をサポートしない指標の場合は`-`と表示されます。発生日時、終了日時でテーブルを並べ替えることができます。

> 参考：持続時間は、通知発生時点から通知終了時点までの時間を示します。
> 通知が発生した状態で、その通知が終了する前に通知を無効にすると、明示的に通知が終了していないため、持続時間が継続的に増加する可能性があります。

<a id="notification-history-view-the-history-of-notification-occurrences"></a>
#### 通知発生履歴照会

![通知発生履歴照会検索フィルタ](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_02_a-1.png)

通知名、通知状態、指標項目、発生時間フィルタを使用して通知発生履歴を照会できます。**初期化**を押すと、検索フィルタが初期化されます。

<a id="notification-history-view-notification-send-history-modal"></a>
#### 通知送信履歴表示モーダル

![通知発生履歴照会画面](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_02-1.png)
各通知で**通知送信履歴を表示**列の**表示**をクリックすると、**通知送信履歴を表示**モーダルが開きます。

![通知送信履歴表示モーダル](https://static.toastoven.net/prod_cloud_monitoring/cloud_monitoring_02_02_b-1.png)

通知送信履歴表示モーダルでは、該当通知の**発生日時**、**継続時間**、**指標項目**、**集計**、**結果値**、**通知条件**などの通知設定情報と**発生位置**を詳細に確認できます。

通知を実際に送信した履歴も表形式で提供します。各通知送信履歴の通知送信日時、通知方法、受信対象、通知送信結果を確認できます。
通知送信日時、通知方法でテーブルのソートを変更できます。

<a id="anomaly-detection"></a>
## 異常検知 { #anomaly-detection }

**Monitoring > Cloud Monitoring > 異常検知**は、Cloud Monitoring が収集した指標パターンを学習し、通常とは異なる異常兆候を自動的に検出します。

しきい値を手動で設定する必要がある通常の通知とは異なり、異常検知は指標の過去のパターンを基準として、Score と Score のしきい値を各時点で自動的に計算します。Score と Score のしきい値はいずれも 0〜100 の範囲の値であり、Score が Score のしきい値を超えると異常状態と判断されます。

異常検知は次の手順で使用します。

1. 異常検知する指標とリソースを選択し、異常検知項目を作成します。
2. ウィジェットチャートで Score と Score のしきい値を確認し、異常が発生した区間を把握します。
3. 異常検知通知を設定することで、異常発生時に通知を受信できます。

!!! tip "ヒント"
    作成した異常検知項目は 1 分間隔で評価されます。

<a id="create-anomaly-detection"></a>
### 異常検知の作成 { #create-anomaly-detection }

**[異常検知の作成]** をクリックすると、異常検知の作成画面に移動します。異常検知のサービスと対象を選択して作成できます。

- **[サービス]**: 異常検知をサポートするNHN Cloudサービスのみ表示され、単一選択のみ可能です。
- **[対象]**: 以下の項目の順序で設定します。
  - **[タイプ]**: サービスの指標タイプを選択します。
  - **[指標項目]**: 選択したタイプに属する指標項目が表示され、複数選択できます。指標項目ごとに選択したリソース数が併せて表示されます。
  - **[リソース]**: NHN Cloud内に作成されたリソースのうち、該当するタイプ/指標項目に属するリソースが表示され、複数選択できます。

選択した指標項目とリソースの組み合わせごとに異常検知項目が1つずつ作成され、異常検知名は選択した指標項目とリソースを基準に自動生成されます。

すでに作成済みの組み合わせでは追加作成できません。このような場合は **[作成済み]** と表示されます。

**[作成項目プレビュー]** で作成される異常検知項目の名前、指標、リソースを表で事前に確認できます。
**[作成]** が完了すると、プレビューに表示された項目がそれぞれ個別の異常検知項目として作成されます。このとき、作成直後にそれらの項目の通知設定画面にすぐ移動するかどうかを選択できます。

!!! danger "注意"
    作成は全体成功または全体失敗として処理されます。
    選択した項目のうち1つでも作成できない項目があると、どの項目も作成されません。

異常検知の作成直後の状態は **有効化-待機** であり、異常検知のための学習および推論が完了すると **有効化-正常** 状態に変更されます。状態が有効化-正常に変更されるまでは、チャートに Score と Score のしきい値が表示されない場合があります。
詳細については、以下の異常検知ステータスを参照してください。

<a id="anomaly-detection-list"></a>
### 異常検知リスト { #anomaly-detection-list }

異常検知画面は、左側のリスト領域と右側のダッシュボード領域で構成されます。リストの異常検知項目が、右側のダッシュボードにそれぞれ1つのウィジェットとして表示されます。

リストの各項目では、異常検知名、サービス、作成日時、ステータス情報を確認できます。項目のチェックボックスは、削除など複数の項目を一度に処理する場合に使用します。

- フィルター、並べ替え、検索は、左側のリストと右側のダッシュボード領域に同時に適用されます。
- **[フィルター]**: サービスと対象フィルターで、リストとダッシュボードに表示する項目をフィルタリングできます。フィルターは、作成された異常検知項目を基準に構成されます。たとえば、Instanceサービスの項目のみある場合、サービスフィルターにはInstanceのみ表示されます。
- **[並べ替え]**: 最新順または古い順で並べ替えることができます。
- **[検索]**: 異常検知名で検索できます。


<a id="anomaly-detection-dashboard"></a>
### 異常検知ダッシュボード { #anomaly-detection-dashboard }

異常検知ダッシュボードには、作成されたすべての異常検知項目が個別のウィジェットとして自動構成されます。

- **[アクティブ（正常）状態のみ表示]** を選択すると、**アクティブ-正常**状態の項目のウィジェットのみ表示されます。
- **[ウィジェットデータのダウンロード]** では、ダッシュボードに表示されたウィジェットデータを .csv または .xlsx ファイルとしてダウンロードできます。

!!! tip "ヒント"
    異常検知ダッシュボードだけでなく、ダッシュボードタブで直接構成したダッシュボードにも異常検知ウィジェットを追加できます。

<a id="anomaly-detection-dashboard-anomaly-detection-widget-chart"></a>
#### 異常検知ウィジェットチャート

ウィジェットチャートには、次の要素が表示されます。

- **指標値**: 異常検知の対象として選択した指標の実際の収集値です。左側のY軸を基準に表示されます。
- **Score**: 現在のデータが過去のパターンから外れた程度を 0〜100 に換算した数値です。100 に近いほど、通常のパターンと異なる異常兆候を意味します。右側のY軸を基準に表示されます。
- **Score のしきい値**: Score がしきい値を超えると、異常と判断します。Score と同じ右側のY軸を基準に表示されます。

Score が Score のしきい値を超えた区間が、異常状態と判断された区間です。

<a id="anomaly-detection-dashboard-delete-anomaly-detection"></a>
#### 異常検知の削除

異常検知リストで選択した項目を削除できます。

- 削除した項目と同じ指標項目およびリソースの組み合わせで再作成すると、以前に削除された項目と連動していたウィジェットおよび通知に自動的に再連動されます（ただし、データ確保のプロセスが再度実行される場合があります）。

!!! danger "注意"
    異常検知項目を削除しても、該当項目を使用しているウィジェットや通知は自動的に削除されません。
    削除された項目を使用しているウィジェットには異常検知データが表示されなくなり、通知も発生しなくなるため、使用しない場合は手動で整理する必要があります。

<a id="anomaly-detection-dashboard-set-up-anomaly-detection-activation"></a>
#### 異常検知の有効化設定

異常検知リストで選択した項目の有効/無効を変更できます。

- **有効化**: 異常検知を作成すると、デフォルトで有効に自動設定されます。無効状態から有効に切り替えた場合、データの確保が再度行われる場合があります。
- **無効化**: 該当項目の異常検知が即座に停止され、その異常検知で作成したウィジェットに異常検知データが表示されなくなり、通知も発生しなくなります。

<a id="anomaly-detection-dashboard-set-up-anomaly-detection-notifications"></a>
#### 異常検知通知設定

異常検知リストで選択した項目の通知を設定できます。

- 選択した項目が設定された状態で通知作成画面に移動します。通知は1件につき1つのサービス指標で作成できるため、同一サービスの異常検知項目を選択した場合にのみ使用できます。


<a id="anomaly-detection-status"></a>
### 異常検知ステータス { #anomaly-detection-status }

| ステータス | 説明 | 
| --- | --- |
| 有効化-正常 | - 異常検知が有効化され、正常に動作している状態です。 <br> - ウィジェットに異常検知データが表示され、通知が発生します。  |
| 有効化-待機中 | - 異常検知が有効化され、データの確保を進めている状態です。 <br> - 異常検知の学習および推論に必要なデータを確保するまで、約 6 時間かかる場合があります。データの確保が完了した時点から異常検知データが表示され、通知が発生します。 | 
| 有効化-データ不足 | - 異常検知対象の収集指標データ不足により、異常検知が動作していない状態です。 <br> - サービス障害やリソースの削除などにより発生する可能性があります。各サービスのコンソールでリソースの状態を確認してください。 <br>（リソースが削除されている場合は、該当の異常検知設定を削除してください。問題がないにもかかわらず該当の状態が続く場合は、カスタマーサポートにお問い合わせください。） | 
| 有効化-一時停止 | - 一時的な問題により、異常検知の動作が一時停止している状態です。 <br> - システムが自動的に復旧します。復旧完了後、分析のための追加データ確保が行われる場合があります。 <br>（該当の状態が長時間続く場合は、カスタマーセンターにお問い合わせください。） |
| 無効化 |  - 異常検知が無効化され、動作していない状態です。 <br> - 有効化設定により、異常検知を再度動作させることができます。 <br>（なお、有効化の際には追加データ確保のための時間が必要です。データの確保が完了した後から、異常検知データの表示および通知が発生します。） | 

!!! tip "ヒント"
    有効化-正常状態を除くすべての状態では、異常兆候データは発生せず、これに伴い通知も発生しません。
    有効化-正常以外の状態（有効化-待機、有効化-データ不足）で一時停止が発生すると、学習および推論のためのデータ確保プロセスが初期化される場合があります。この場合、一時停止前の状態によって、最低6時間以上の追加時間が必要になる場合があります。

<a id="use-anomaly-detection"></a>
### 異常検知の活用 { #use-anomaly-detection }

<a id="use-anomaly-detection-add-anomaly-detection-widget"></a>
#### 異常検知ウィジェット追加

自分で作成したダッシュボードにも、異常検知ウィジェットを自由に追加できます。

- ウィジェット追加画面で、**[対象タイプ]** で **[異常検知]** を選択します。
- **[グラフタイプ]** は `Anomaly Detection` に固定されます。
- サービスを選択すると、そのサービスで作成された異常検知の一覧が表示されます。目的の異常検知項目が一覧にない場合は、異常検知タブで先に作成する必要があります。
- 選択した異常検知項目ごとに、指標設定ブロックが1つずつ追加されます。
- クエリ設定ブロックには、指標値、Score、Score しきい値の3つの凡例が自動的に配置されます。異常検知で作成したウィジェットは、常にこの3種類のデータが一緒に表示されます（ただし、状態によって表示されない場合があります）。Score と Score しきい値の凡例名・単位・Y軸位置は固定値です。
- 集計をサポートする指標では集計設定を使用でき、設定した集計は指標値・Score・Score しきい値のすべてに適用されます。

!!! danger "注意"
    ウィジェットで使用中の異常検知項目が削除された場合、該当ウィジェットの照会と保存は可能ですが、削除された項目のクエリ設定値は変更できません。

##### しきい値の直接指定

異常検知はデータポイントごとにしきい値を自動的に判断するため、基準値は時点によって異なります。固定した基準を使用するには、**[しきい値の直接指定]** を選択して値を入力します。

- しきい値は0以上100以下の数値のみ入力できます。
- 直接指定すると、データポイントに関係なく入力した値を基準として異常を判断します。
- 選択を解除すると、異常検知が判断したしきい値に従ってチャートに表示されます。

<a id="use-anomaly-detection-anomaly-detection-notifications"></a>
#### 異常検知通知

異常が検知されたときに通知を受け取るには、異常検知通知を作成します。異常検知通知は、既存の指標通知と同様に、**[通知管理]** タブ > **[通知作成]** 画面から作成できます。

##### しきい値の設定

異常検知はデータポイントごとにしきい値を自動で判断するため、基準値は時点によって異なります。固定された基準を使用するには、条件ごとに **[直接指定]** を選択し、値を入力します。

- しきい値には 0 以上 100 以下の数値のみ入力できます。
- 直接指定すると、データポイントに関係なく、入力した値を基準として異常を判断し、通知が発生します。
- 選択を解除すると、異常検知が判断したしきい値超過の状態が 5 分間継続した場合に通知が発生します。

##### 異常検知通知の確認

- 異常検知通知は、既存の指標通知と同様に、通知設定タブおよび通知発生履歴タブで確認できます。異常検知通知の場合は、別途バッジで区別して表示します。
- 通知発生履歴では、**[対象タイプ]** フィルターで異常検知通知のみ照会できます。

!!! danger "注意"
    通知は、異常検知項目が **[有効-正常]** 状態のときにのみ発生します。データ取得中、無効、データ不足、一時停止の状態では、通知条件を評価しません。項目の状態は **[異常検知]** 画面で確認できます。
    通知で使用中の異常検知項目が削除された場合、該当する通知の照会と保存は可能ですが、削除された項目の通知条件は変更することはできません。


<a id="use-anomaly-detection-notes"></a>
#### 注意事項

- 項目を作成した直後は、異常兆候の検出に向けた学習および推論が完了するまで、Score と Score のしきい値が表示されない場合があります。
- 異常兆候の検出が使用する元の指標データが連続 30 分間収集されない場合、データ不足の状態に変更され、Score と Score のしきい値も生成されなくなります。指標収集が再開されると、状態は「有効化-待機」または「有効化-正常」に復元されます。

<a id="example-screen"></a>
## 例画面 { #example-screen }

![ダッシュボード例](https://static.toastoven.net/prod_cloud_monitoring/Overview.png)
